...
#     for l in 1: (N_sites - 1)
#         for i in 1 : N_sites - l 
#             j = i + 1 
#             #s_i, s_j are non-neighbouring spin site/indices from the s array
#             s_i = s[i]                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        
#             s_j = s[j]
...
#             # Insert SWAP gate after applying Gj
...
#             # i.e. we append a new element Gj (which is an ITensor object representing a gate) to the end of the gates array.
#             push!(gates, Gj)
#             # push!(swap_op(s[i], s[j]) * gates, Gj)
#             swap_op(s_i, s_j)
#             # Multiply the SWAP operator with the first ITensor in gates
#             swap * gates[i]
#             # ψ = movesite(ψ, i=>j)
#             # ψ = apply(swap_op(s[i], s[j]), ψ)
#             # # Insert SWAP gate after applying Gj
#             # swap_gate = swap_op(s_i, s_j)
...
# Define the swap_op function to create a SWAP gate
function swap_op(site1, site2)
    # Construct the SWAP gate as a tensor product of identity and exchange operators
    # swap_op = op("Swap", site1, site2)

    # # Define a 2-site index
    site1 = Index(2, "site1") # site1
    site2 = Index(2, "site2") # site2
    # Manually create the SWAP operator
    swap_op = ITensor(site1', site1, site2', site2)
    swap_op[site1=>1, site1'=>1, site2=>1, site2'=>1] = 1.0
    swap_op[site1=>2, site1'=>2, site2=>2, site2'=>2] = 1.0
    swap_op[site1=>1, site1'=>2, site2=>2, site2'=>1] = -1.0 #changed for 2 particle in Richers b/c half neutrino half anti neutrios in that test, # = 1.0 for Rog case since he takes all neutrinos on the chain (half electron and half muons)
    swap_op[site1=>2, site1'=>1, site2=>1, site2'=>2] = -1.0 # = 1.0 for Rog case since he takes all neutrinos on the chain (half electron and half muons)
    println("SWAP Operator Tensor:")
    print(swap_op)
    return swap_op
end
...
#             # Calculate the shape function result for each pair of i and j 
#             shape_result = shape_func(x, Δp, i, j,L, shape_name, periodic)
#             # Calculate the geometric factor for each pair of i and j within the loop
#             geometric_factor = geometric_func(p, p̂, i, j, theta_nu)
#             interaction_strength = (2.0 * √2 * G_F * (N[i] + N[j]) / (2 * ((Δx)^3))) * shape_result * geometric_factor
...
#             if ω[i] != 0 || ω[j] != 0
#                 hj += (1/(N_sites-1)) * (
#                         (ω[i] * B[1] * op("Sx", s_i) * op("Id", s_j)) +
#                         (ω[i] * B[2] * op("Sy", s_i) * op("Id", s_j)) +
#                         (ω[i] * B[3] * op("Sz", s_i) * op("Id", s_j)))
#                 hj += (1/(N_sites-1)) * (
#                         (ω[j] * B[1] * op("Id", s_i) * op("Sx", s_j)) +
#                         (ω[j] * B[2] * op("Id", s_i) * op("Sy", s_j)) +
#                         (ω[j] * B[3] * op("Id", s_i) * op("Sz", s_j)))
#             end
    
#             # make Trotter gate Gj
#             if theta_nu == 0 || theta_nu == π/4 || theta_nu == π/2 
#                 Gj = exp(-im * τ / 2 * hj)
#                 # Gj = exp(-im * τ * hj)
#             elseif theta_nu == 0.1 && Δm² == 0.2  # for Rog_bipolar
...
#             end
#             println(Gj)
#             # Append the gate to the gates array
#             push!(gates, Gj)
#             # has_fermion_string(hj) = true
#             # swapbondsites(ψ, s_i) # swaps indices s[i] and s[i+1] = s[j]
#             # Now implement the SWAP operation after each Gij
...
#             # Swap the physical indices
#             temp = s[i]
#             s[i] = s[j]
#             s[j] = temp

#             # println("After swapping s = ", s)

#             # Define Physical Quantum Numbers
#             spin_up_qn = QN("Sz" => +1)
#             spin_dn_qn = QN("Sz" => -1)

#             # Create a new link index with appropriate quantum number
#             # Use the correct Index constructor for QN-conserving indices
#             link_ind = Index([spin_up_qn => 1]; tags="Link,l=1")

#             # Create tensors with swapped physical indices and spin states
#             # Site i Tensor
#             A1 = ITensor(s[i], link_ind)
#             A1[s[i] => "Up", link_ind => 1] = 1.0

#             # Site j Tensor
#             A2 = ITensor(prime(link_ind), s[j])
#             A2[prime(link_ind) => 1, s[j] => "Dn"] = 1.0

#             # Build the new MPS
#             ψ = MPS([A1, A2])
#             println("After swapping ψ_new = ", ψ)
    
#             # # Optionally print to check swap
#             println("Swapped indices: i = $i, j = $j")
#         end 
#     end
