...
function main()
  ...
  gates = create_gates(s, n, ω, B, N, del_x, tau)
  
  # Initialize psi to be a product state (alternating up and down)
  psi = productMPS(s, n -> isodd(n) ? "Up" : "Dn")

  #extract output from the expect.jl file where the survival probability values were computed at each timestep
  Sz_array, prob_surv_array = evolve(s, tau, n, ω, B, N, del_x, psi, cutoff, tolerance, ttotal)
  # Plotting P_surv vs t
  plot(0.0:tau:tau*(length(Sz_array)-1), Sz_array, xlabel = "t", ylabel = "<Sz>", legend = false, size=(800, 600), aspect_ratio=:auto,margin= 10mm) 
