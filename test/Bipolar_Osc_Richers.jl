...
function main()
    ...
    params = CCNO.Parameters(
        ...
        Delta_p = L, # width of shape function  # cm #variable
        Delta_x=Delta_x,
        L=L,
    ...
    Delta_m_squared = (params.m2^2-params.m1^2) # mass square difference # (erg^2) #not used in the test
    Enu_e =  50.0*CCNO.MeV #ergs # energy of all neutrinos (P.S the its negative is energy of all antineutrinos) 
    ...
    n_nu_e =  (10* (params.m2-params.m1)^2 )/(2*(√2)*CCNO.G_F*Enu_e) # cm^-3 # number density of electron flavor neutrino
    ...
    # n_nu_e,n_nu_ē,Enu_e,Enu_ē,Delta_x,Delta_m_squared, p, x, Delta_p, Psi₀, shape_name, energy_sign, cutoff, maxdim, ttotal,periodic)
    ...
    if params.save_plots_flag 
        ...
        CCNO.save_plots(params, L,t_array, Sz_array, Sy_array, Sx_array, x_values, pₓ_values, rho_e_e_array,rho_mumu_array, rho_emu_array)
        # Call the function to generate the inputs file in the specified directory
        CCNO.generate_inputs_file(params, "inputs.txt")
    end
    if !params.save_plots_flag
        # Plotting rho_ee vs t # for jenkins file 
        plot(t_array/t_bipolar, rho_e_e_array, xlabel = "t", ylabel = "<rho_e_e>", legend = false, 
        left_margin = 20mm, right_margin = 10mm, top_margin = 5mm, bottom_margin = 10mm) 
        # Save the plot as a PDF file
        savefig("Bipolar Richers_site1 for $(params.N_sites) particles <rho_e_e>_vs_t.pdf")
    end
