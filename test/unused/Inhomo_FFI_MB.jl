...
function main()
    ...
    k = 2*pi / (L)
    analytic_growth_rate=  (abs(params.m2^2 - params.m1^2)/ (2*CCNO.hbar* Enu_e)) + (CCNO.c* k)  # analytic growth rate #fix it for inhomo from paper
    ...
    if params.save_plots_flag 
        ...
        rho_e_e_array_domain_avgd = [mean(abs.(row)) for row in eachrow(rho_e_e_array)]
        rho_mumu_array_domain_avgd = [mean(abs.(row)) for row in eachrow(rho_mumu_array)]
        rho_emu_array_domain_avgd= [mean(abs.(row)) for row in eachrow(rho_emu_array)]
        ...
        CCNO.save_plots(params, L,t_array, Sz_array_domain_avgd, Sy_array_domain_avgd, Sx_array_domain_avgd, x_values, pₓ_values, rho_e_e_array_domain_avgd,rho_mumu_array_domain_avgd, rho_emu_array_domain_avgd)
    ...
    if !params.save_plots_flag
        ...
        plot(t_array, rho_emu_array_domain_avg, xlabel = "t", ylabel = "<rho_emu>", legend = false, 
        left_margin = 20mm, right_margin = 10mm, top_margin = 5mm, bottom_margin = 10mm) 
        # Save the plot as a PDF file
        savefig( "Inhomo_MB_<rho_emu>_domain_avg_vs_t for $(params.N_sites) particles.pdf")
    end
